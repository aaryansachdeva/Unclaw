// Strands — woven, glowing WebGL strands (React Bits, ogl). Adapted to TS.
// Used as the live, voice-reactive fill of the continuous voice-mode orb:
// the host passes amplitude/speed/intensity derived from the mic VAD level so
// the strands physically ripple with the user's voice. Reads live values from
// a ref every frame, so prop changes (60fps vadLevel) react without remounting.
...
const buildPalette = (colors: string[]): RGB[] => {
...
export default function Strands({
  ...
  const propsRef = useRef<Required<Omit<StrandsProps, 'className' | 'style'>>>({} as never);
  propsRef.current = {
